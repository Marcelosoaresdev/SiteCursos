...
function LearnMoreButton({ text, variant = 'dark' }) {
  ...
  const initialTextColor = variant === 'light' ? 'text-white' : 'text-[#282936]';
  
  const hoverTextColor = variant === 'light' ? 'group-hover:text-saude-dark-blue' : 'group-hover:text-white';
  const arrowColor = variant === 'light' ? 'before:border-saude-dark-blue' : 'before:border-white';
  const arrowHoverBg = variant === 'light' ? 'group-hover:bg-saude-dark-blue' : 'group-hover:bg-white';
  
  return (
    <button className="group relative w-32 h-8 cursor-pointer border-none bg-transparent p-0 flex items-center">
